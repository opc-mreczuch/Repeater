...
@Composable
fun HomeScreen(taskDataStore: TaskDataStore) {
    val tasks by taskDataStore.homeTasksFlow.collectAsState(initial = emptyList())
    ...
    fun addTask() {
        if (taskName.isNotEmpty()) {
            ...
            coroutineScope.launch {
                taskDataStore.saveHomeTasks(tasks + newTask)
            }
            taskName = ""
        }
    }
    ...
    fun deleteTask(task: Task) {
        coroutineScope.launch {
            taskDataStore.saveHomeTasks(tasks.filter { it != task })
        }
        taskToDelete = null
    }
    ...
    fun moveTask(from: Int, to: Int) {
        ...
        coroutineScope.launch {
            taskDataStore.saveHomeTasks(updatedTasks.mapIndexed { index, task ->
                ...
                TaskItem(
                    ...
                    onTaskClick = {
                        coroutineScope.launch {
                            ...
                            taskDataStore.saveHomeTasks(updatedTasks)
                        }
                    },
                    onDeleteClick = {
                        taskToDelete = task
